...
export async function initOnboardingWizard() {
  ...
  const injuriesList =
    Array.isArray(injuryOptions) && injuryOptions.length
      ? injuryOptions.map((item) => ({ value: item.code, label: item.label }))
      : [];
  ...
  const getDefaultWheelValue = (step) => {
    if (step.defaultValue !== undefined) return String(step.defaultValue);
    const midpoint = (step.min + step.max) / 2;
    const isFloat = Math.abs(step.step % 1) > 0;
    if (isFloat) {
      return (Math.round(midpoint / step.step) * step.step).toFixed(1);
    }
    return String(Math.round(midpoint));
  };
  ...
  const setValue = (step, raw) => {
    if (step.type === "wheel" || step.type === "wheel-horizontal") {
      const isFloat = Math.abs(step.step % 1) > 0;
      const val = isFloat ? Number.parseFloat(raw) : Number(raw);
  ...
  const renderStep = () => {
    ...
    descEl.textContent =
      step.type === "wheel" || step.type === "wheel-horizontal"
    ...
    if (step.type === "multi-options") {
      ...
      step.options.forEach((opt) => {
        ...
        // текст слева, пин справа (как у тебя в CSS)
    ...
    if (step.type === "goal-combo") {
      ...
      step.options.forEach((opt, index) => {
        const input = document.createElement("input");
        input.type = "radio";
        input.name = `onboarding-tabs-${step.id}`;
        input.id = `onboarding-${step.id}-${index}`;
    ...
    if (step.type === "wheel" || step.type === "wheel-horizontal") {
      ...
      if (data[step.id] === undefined) setValue(step, initial);

      const axis = step.type === "wheel-horizontal" ? "x" : "y";
      const extraClass =
        step.type === "wheel-horizontal" ? "horizontal" : step.variant === "side" ? "side" : "";
  ...
  const validateStep = (step) => {
    ...
    if (step.type === "options" || step.type === "sex-buttons") {
  ...
  const submitOnboarding = async () => {
    const stageMap = { core: 0, flow: 3, peak: 6 };
    ...
    // генерацию пока не включаем: trainingInput будет null
    const trainingInput = null;
