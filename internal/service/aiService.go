...
Всегда отвечай на русском языке.`
	...
	— используй блок normalized из профиля как источник ключевых параметров
	— сначала определить training_days_per_week и выбрать недельный шаблон (split):
	  - fullbody при 2–3 днях
	  - upper/lower при 4 днях
	  - PPL при 5–6 днях, если нет проблем со спиной/локтями/плечами
	  Затем только заполнять week_plan
	...
	— в каждом дне нужен type: "train" или "rest"
	— если type="train": 5–7 пунктов в items
	— если type="rest": 1–2 пункта в items (например: "Отдых", "Ходьба 30–45 мин", "Мобилити 10 мин")
	— никаких переносов строк внутри строк
	— ротация упражнений и запрет дублей:
	  - одно и то же базовое упражнение максимум 1 раз в неделю (2 раза только если цель — сила)
	  - нельзя ставить 2 упражнения с одинаковым паттерном в один день
	  - ротируй паттерны: горизонтальный жим (штанга/гантели/машина), вертикальная тяга (подтягивания/верхний блок), горизонтальная тяга (гантель/гребля/блок),
	    сгибание/разгибание ног (жим ногами/сгибания/разгибания/гиперэкстензии)
	— осевая нагрузка при проблемах со спиной:
	  запрет: тяжёлая становая с пола, good morning, присед со штангой на спине, тяжёлые наклоны со штангой
	  предпочтение: жим ногами, болгарские, гакк/смит умеренно, гиперэкстензия без веса/лёгкая, тяги с грудной опорой, блочные тяги
	— в тренировочный день структура: 1–2 главных лифта, 2–3 вспомогательных, 1–2 изоляции/кор
