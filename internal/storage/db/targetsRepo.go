...
func (r *TargetsRepo) Upsert(ctx context.Context, t domain.Targets) error {
	_, err := r.db.Exec(ctx, `
		...
		on conflict (chat_id) do update set
			...
			updated_at=now()
	`, t.ChatID, t.Kcal, t.ProteinG, t.FatG, t.CarbsG, t.Steps, t.Source)
	return err
}
...
func (r *TargetsRepo) Get(ctx context.Context, chatID int64) (domain.Targets, bool, error) {
	...
	err := r.db.QueryRow(ctx, `
		select kcal_target, protein_g, fat_g, carbs_g, steps_target, source
		from user_targets
		where chat_id=$1
	`, chatID).Scan(&t.Kcal, &t.ProteinG, &t.FatG, &t.CarbsG, &t.Steps, &t.Source)
...
func (r *TargetsRepo) SetManualField(ctx context.Context, chatID int64, field string, value int) error {
	...
	case "steps":
		q = `update user_targets set steps_target=$2, source='manual', updated_at=now() where chat_id=$1`
	default:
